...
class PositionControl:
    def __init__(
        ...
        **kwargs,
        ...
        if self.debug:
            self.controller_record = deque(
                maxlen=self.info.horizon * self._n_intermediate_steps
            )
    ...
    def _enforce_safety_limits(self, desired_pos, desired_vel):
        ...
        joint_pos_lim = np.tile(
            self.env_info["robot"]["joint_pos_limit"], (1, self.n_agents)
        )
        joint_vel_lim = np.tile(
            self.env_info["robot"]["joint_vel_limit"], (1, self.n_agents)
        )

        min_vel = np.minimum(
            np.maximum(-k * (pos - joint_pos_lim[0]), joint_vel_lim[0]),
            joint_vel_lim[1],
        )

        max_vel = np.minimum(
            np.maximum(-k * (pos - joint_pos_lim[1]), joint_vel_lim[0]),
            joint_vel_lim[1],
        )
    ...
    def _controller(
        self, desired_pos, desired_vel, desired_acc, current_pos, current_vel
    ):
        ...
        torque = (
            self.p_gain * error
            + self.d_gain * (clipped_vel - current_vel)
            + self.i_error
        )
        ...
        for i in range(self.n_agents):
            ...
            torque[robot_joint_ids] = np.minimum(
                np.maximum(
                    torque[robot_joint_ids], self.robot_model.actuator_ctrlrange[:, 0]
                ),
...
class PositionControlPlanar(PositionControl):
    def __init__(self, *args, **kwargs):
        ...
        d_gain = [60, 20, 4]
        i_gain = [0, 0, 0]
        super(PositionControlPlanar, self).__init__(
            p_gain=p_gain, d_gain=d_gain, i_gain=i_gain, *args, **kwargs
        )
...
if __name__ == "__main__":
    ...
    env = IiwaPositionHit(
        viewer_params={"start_paused": True}, debug=True, interpolation_order=None
    )
    env.reset()
    steps = 0
    ...
    while True:
        ...
        if steps * env.dt <= phase_time:
            ...
            pos = (
                init_pos
                - 1 / 2 * max_acc * phase_time**2
                - max_vel * (steps * env.dt - phase_time)
            )
            ...
            pos = (
                ...
                - 1 / 2 * max_acc * phase_time**2
                - max_vel * phase_time
                - max_vel * (steps * env.dt - 2 * phase_time)
                + 1 / 2 * max_acc * (steps * env.dt - 2 * phase_time) ** 2
            )
            ...
            pos = (
                init_pos
                - 2 * max_vel * phase_time
                + 1 / 2 * max_acc * (steps * env.dt - 4 * phase_time) ** 2
            )
            ...
            pos = (
                ...
                - 2 * max_vel * phase_time
                + 1 / 2 * max_acc * phase_time**2
                + max_vel * (steps * env.dt - 5 * phase_time)
            )
            ...
            pos = (
                ...
                + 1 / 2 * max_acc * phase_time**2
                + max_vel * phase_time
                + max_vel * (steps * env.dt - 6 * phase_time)
                - 1 / 2 * max_acc * (steps * env.dt - 6 * phase_time) ** 2
            )
        else:
            acc = np.zeros(nq)
            vel = np.zeros(nq)
        ...
        if done or steps >= 200:
            if env.debug:
                ...
                for j in range(nq):
                    ...
                    axes[4, j].plot(
                        trajectory_record[:, j + nq_total] - trajectory_record[:, j]
                    )
                plt.show()
            steps = 0
            env.reset()
